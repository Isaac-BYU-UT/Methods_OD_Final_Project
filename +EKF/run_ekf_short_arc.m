...
for i = 2:ekf.N_obs
    ...
    if (i_case == 6 && ekf.t_obs(i) >= 5 * Units.SEC_IN_SOLAR_DAY && ~reached_5_days)
        disp("I've Arrived at 5 Days, my state is: ")
        disp(ekf.X_input(1:ekf.N_states))
        SAVE_5_DAYS_STATE = ekf.X_input(1:ekf.N_states);
        fprintf("Time: %d at index %d", ekf.t_obs(i), i);
        reached_5_days = true;
    end
    ...
    ekf.X_input = [X_states_updated; STM_reset];
    ekf.P_cov   = P_cov_updated;
end
...
t_dV1 = datetime(2018,3,30,8,55,3,"TimeZone","UTC") - S.IC_Sat_Epoch.epoch_date_time_UTC; % This gives a duration object
t_dV1_sec = seconds(t_dV1); % Convert to seconds

if strcmp(ekf.ode_type, 'ode113')

[~, y_final] = ode113(@(t,X) jah_sat_1_ode( ...
