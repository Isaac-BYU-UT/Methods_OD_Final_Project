function [X_states_updated, P_cov_updated, dx, ekf] = ekf_update(ekf, X_states_propogated, P_bar, curr_meas, S, ENV)
    ...
    if update_this_step
        ...
        H_tilde_prefit = Measurements.Compute_H_matrix( r_sat_nominal_ECI_m, v_sat_nominal_ECI_m_s,... % Before update
        ...
        % --- Prefit (Measurement) ---
        ekf.Y_prefit(:,i) = curr_meas.y_computed_propogated_no_lt_meters;

        % lt_guess_initial = curr_meas.y_computed_propogated_no_lt_meters(1)/ ... % Use Range
        %                     PhysicsConstants.C_SPEED_OF_LIGHT_M_S;
        % 
        % 
        % ekf.Y_prefit(:,i) = curr_meas.y_lt_computed_propogated_meters;
        curr_meas.residual = curr_meas.y_obs_meters - curr_meas.y_computed_propogated_no_lt_meters; % In meters and meters per second
        
        % curr_meas.y_lt_computed_propogated_meters = Measurements.LightTimeCorrection();
        ...
        r_sat_updated_ECI_m = X_states_updated(1:3);
        v_sat_nominal_ECI_m_s = X_states_updated(4:6);

        % ---- POSTFIT (Covariance) ----

        H_tilde_postfit = Measurements.Compute_H_matrix( ...
                                                r_sat_updated_ECI_m, v_sat_nominal_ECI_m_s,...
        ...
        ekf.Y_postfit(:,ekf.current_index) = curr_meas.y_computed_filtered_no_lt_meters;
        % curr_meas.y_lt_filtered_propogated_meters = Measurements.LightTimeCorrection();
        % ekf.Y_postfit(:,ekf.current_index) = curr_meas.y_lt_filtered_filtered_meters;
